		 ...
		 PrintWriter out = response.getWriter();
			...
			connection = DriverManager.getConnection("jdbc:mysql://","root", "decodder");
			 ...
			 user = request.getParameter("user");
				...
				out.println("<HEAD><TITLE>Add / Remove privileges for "+user+"</TITLE></HEAD>");
				

				 out.println(""
				 		+ " "
						 + "<br>"
				 		+ "<H2 ALIGN=\"CENTER\">Add / Remove privileges for "+user+"  </H2><FORM =\"/UserManagement/\" METHOD=\"GET\">"
				 		+ ""
				 		+ " <left><button class='login_btn' type='submit' style='font-size:20px;'>"
					+ "<img src='http://goo.gl/wwTkAq?gdriveurl' height='24' width='24'>Next</button></left><br><br>");

				 if(message !=null)
					 out.println("<br><div><center><span style=\"color:Black;fonts-size:50px;font-weight:bold;\">" + message + "</span></center>=");
...
///                      DB names
					...
					// Perform the query
					 ...
					 out.println(" <td> <input type=\"radio\" name=\"dbname\" value=\"*\" >All</td>");
						while (resultDB.next())
						{
							...
							if((dbName.compareTo("information_schema")) !=0  &&   (dbName.compareTo("performance_schema")!=0)&&   (dbName.compareTo("mysql")!=0) )
							
							{   
								if(temp !=null && temp.compareTo(dbName) ==0)
								out.println(" <td> <input type=\"radio\" name=\"dbname\" value="+dbName+" checked>"+dbName+"</td>");
								else								
								 out.println(" <td> <input type=\"radio\" name=\"dbname\" value="+dbName+" >"+dbName+"</td>");
				...
				if(execute ==null)	{	
						...
						do{
							...
							while (resultTB.next() )
							{
								...
								out.println(" <td> <input type=\"radio\" name=\"tblname\" value="+tblName+" checked>"+tblName+"</td>");
								else	
									out.println(" <td> <input type=\"radio\" name=\"tblname\" value="+tblName+" >"+tblName+"</td>");
		...
		//  Column names
				...
				database =  request.getParameter("dbname");
						if(database !=null)
						{
							...
							ResultSet ColData= myTBStm.executeQuery("describe "+tblName);
							...
							while (ColData.next() )
							{
								...
								out.println("<td> <input type=\"radio\" name=\"colname\" value="+colName+">"+colName+"</td>");
				  ...
				  if(colName !=null){
					  if(colName.contains("*")){
						  ...
						  privilegesOnDB(request, response, dbName, tblName,colName);
				  }
				}else{
					...
					do{
						...
						//Create and execute an SQL statement to get all the table names in moviedb
						Statement myTBStm = connection.createStatement();
						ResultSet resultRT = myTBStm.executeQuery("show procedure status");
						ResultSet ColData;
						Statement myColStm;
						out.println("<td>" );
						count_tb =0;
						...
						while (resultRT.next() )
						{
							...
							out.println(" <td> <input type=\"radio\" name=\"proc\" value="+rtName+" checked>"+rtName+"</td>");
							else	
								out.println(" <td> <input type=\"radio\" name=\"proc\" value="+rtName+" >"+rtName+"</td>");

						}
						
						out.println(" </tr>");
						Statement myfunct = connection.createStatement();
						ResultSet resultFt = myfunct.executeQuery("show function status");
						...
						while (resultFt.next() )
						{
							...
							if(temp !=null && temp.compareTo(rtName) ==0)
							out.println(" <td> <input type=\"radio\" name=\"func\" value="+ftName+" checked>"+ftName+"</td>");
							else	
								out.println(" <td> <input type=\"radio\" name=\"func\" value="+ftName+" >"+ftName+"</td>");
    ...
    void privilegesOnDB(HttpServletRequest request, HttpServletResponse response) throws IOException {
		 String query=" ";
		 grant=request.getParameter("prev");
		  query +=query+ grant;
		  String to_f=null;
		...
		to_f=" to ";
		  ...
		  if(all !=null){
		   ...
		   try {
			...
			if(rs==0){
				 response.sendRedirect("/UserManagement/addUser.html?user="+user+"&message=sussfully updated all privileges"); 
			}
		} catch (SQLException | IOException e) {
			 response.sendRedirect("/UserManagement/addUser.html?user="+user+"&message="+query+e); 

			e.printStackTrace();
		}
		 
	}
	void privilegesOnDB(HttpServletRequest request, HttpServletResponse response, String dbName) throws IOException {
		 String query=" ";
		 grant=request.getParameter("prev");
		  query +=query+ grant;
		  String to_f=null;
		...
		to_f=" to ";
		  ...
		  if(all !=null){
		   ...
		   if(insert!=null)
			   query+=" , "+insert;
		   query = query.replaceFirst(",", " ");}
		   query =query+" on "+dbName+".* "+to_f+" '"+user+"'@'localhost'";
	...
	void privilegesOnDB(HttpServletRequest request, HttpServletResponse response, String dbName, String tblName) throws IOException {
		 String query=" ";
		 grant=request.getParameter("prev");
		  query +=query+ grant;
		  String to_f=null;
		...
		to_f=" to ";
		  ...
		  if(all !=null){
		   ...
		   if(insert!=null)
			   query+=" , "+insert;
		   query = query.replaceFirst(",", " ");}
		   query =query+" on "+dbName+"."+tblName+" "+to_f+" '"+user+"'@'localhost'";
	...
	void privilegesOnDB(HttpServletRequest request, HttpServletResponse response, String dbName, String tblName, String colName) throws IOException {
		 String query=" ";
		 grant=request.getParameter("prev");
		  query +=query+ grant;
		  String to_f=null;
	...
	void privilegesOnProc(HttpServletRequest request, HttpServletResponse response, String proc) throws IOException {
		 String query=" ";
		 grant=request.getParameter("prev");
		  String to_f=null;
		  if(grant.contains("grant"))
		to_f=" to ";
		   ...
		   query =grant+" execute on procedure "+proc+" "+to_f+" '"+user+"'@'localhost'";
		   ...
		   try {
			...
			void privilegesOnFunc(HttpServletRequest request, HttpServletResponse response, String func) throws IOException {
				 String query=" ";
				 grant=request.getParameter("prev");
				 String to_f=null;
				  if(grant.contains("grant"))
				to_f=" to ";
					  ...
					  to_f=" from ";
				   query =grant+" execute on function sakila."+func+" "+to_f+"'"+user+"'@'localhost'";
